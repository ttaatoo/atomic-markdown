...
- **WYSIWYG via selection, not a Word ribbon.** A compact floating format bar appears when text is selected in edit mode. It hides when the selection collapses, in reading mode, or when focus is on a real chrome control (outline, notices). Keyboard shortcuts still format with no chrome on screen.
