...
- **Selection → Cursor Chat.** Capsule lightning or card **Send** (empty comment allowed) ships the selection. A typed note adds `Comment: …`. Fixed top **Global comment** sends an optional note plus the file (fenced when small). **Copy** writes the full document to the host clipboard. Not an in-editor AI assistant and not native reference chips.
