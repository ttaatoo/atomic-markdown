...
- Selection format bar is a CodeMirror 6 `showTooltip` extension (`EXTRA_EXTENSIONS`), not a React overlay. It shows when `state.selection.main` is non-empty and the editor is not read-only (`readOnlyFacet` / `state.readOnly`). No `document.activeElement` gate. Shortcuts still format with no chrome.
