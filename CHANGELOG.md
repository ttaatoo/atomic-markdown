...
- Selection bar QA: show whenever CM `sel.empty` is false in edit mode (BODY/`hasFocus` flicker is not a hide). Place from `coordsAtPos` (side-biased, throw-safe), then `.cm-selectionBackground` / selection-layer rects, then `domAtPos` Range, then a non-collapsed `window.getSelection()`, then contentDOM / a safe default — never omit the bar because coords failed. Refresh on mouseup, shift-selection keyup, and `selectionchange`; z-index 50.
