...
- Selection bar QA: show whenever CM `sel.empty` is false in edit mode. Hide only for real chrome focus (outline item/button/twisty, notice dismiss) — not `#root` / `.app` / `.editor-frame` / BODY / `hasFocus` flicker. Place from `coordsAtPos` (side-biased, throw-safe), then `.cm-selectionBackground` / selection-layer rects, then `domAtPos` Range, then a non-collapsed `window.getSelection()`, then contentDOM / a safe default. Refresh on mouseup, shift-selection keyup, and `selectionchange`; z-index 50.
