...
- Selection → Cursor Chat: card **Send** is enabled with an empty comment (ships the selection). Lightning is one-click empty send. A typed note adds `Comment: …`. Top **Global comment** sends an optional note plus the file (fenced under ~8k chars). Top **Copy** asks the host to clipboard `document.getText()` (`copyDocument` → `documentCopied`); the pill shows Copied only after a successful ack. Host prefers `workbench.action.chat.open` (Cursor ≥2.3) and falls back to clipboard + `composer.newAgentChat` + paste. Flex rules honor `[hidden]` via `:not([hidden])`.
