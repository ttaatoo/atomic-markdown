...
There is no top Format strip and no format icons on the selection tooltip. **Global comment** and **Copy** are fixed top pills (not on the selection UI). Selecting text shows a compact CodeMirror capsule: comment, lightning (one-click send), and dismiss. Click comment to open a card with a quoted preview. **Send** is enabled even when the textarea is empty (ships the selection). A typed note adds `Comment: …`. **Global comment** sends an optional note plus the file (fenced when the document is under ~8k characters). **Copy** writes the full document through the extension host clipboard. Escape closes the open card first, then dismisses the capsule. The host opens Cursor Chat via `workbench.action.chat.open`, or clipboard + `composer.newAgentChat` + paste if that command is missing. This fills the chat input; it does not create native Monaco reference chips.
