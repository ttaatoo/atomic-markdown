...
- No top format strip. Quiet **Global comment** and **Copy** pills sit at the top-right of the writing column. Select text to get a compact icon capsule (comment, lightning, dismiss); comment opens a card. Format via shortcuts only: **Cmd/Ctrl+B** bold, **Cmd/Ctrl+I** italic, **Cmd/Ctrl+K** link
...
There is no top Format strip and no format icons on the selection tooltip. **Global comment** and **Copy** are fixed top pills (not on the selection UI). Selecting text shows a compact CodeMirror capsule: comment, lightning (one-click send), and dismiss. Click comment to open a card with a quoted preview. **Send** is enabled even when the textarea is empty (ships the selection). A typed note adds `Comment: …`. **Global comment** sends an optional note plus the file (fenced when the document is under ~8k characters). **Copy** asks the extension host to write the full `TextDocument` to the clipboard (not a webview snapshot). Escape closes the open card first, then dismisses the capsule. The host opens Cursor Chat via `workbench.action.chat.open`, or clipboard + `composer.newAgentChat` + paste if that command is missing. This fills the chat input; it does not create native Monaco reference chips.
