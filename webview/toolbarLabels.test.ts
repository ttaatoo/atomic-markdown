...
import { formatActionTitle, formatStripTitle, modifierGlyph, outlineToggleTitle } from './toolbarLabels.ts';
...
describe('toolbar labels', () => {
  ...
  it('matches outline and format affordance titles to hide/show language', () => {
    assert.equal(outlineToggleTitle(false), 'Show outline');
    assert.equal(outlineToggleTitle(true), 'Hide outline');
    assert.equal(formatStripTitle(false), 'Show formatting');
    assert.equal(formatStripTitle(true), 'Hide formatting');
  });
});
