...
- Feishu outline placement (push rail / icon rail / last-resort overlay), nested heading collapse + prune, YAML frontmatter / thematic-break `---` excluded from headings; no top Format strip; CM `showTooltip` comment capsule (card closed by default) plus fixed top Global comment / Copy; chat prompt builder (empty comment + global fence/path-only) and `sendToChat` / `copyDocument` / `documentCopied` protocol; host full-document clipboard copy; image failure copy
...
11. **Selection capsule + top pills / shortcuts**  
    There must be **no** top Format strip, Reading chip, or outline toggle in the webview. Quiet **Global comment** and **Copy** pills sit at the top-right of the writing column (not on the selection tooltip). Select the plain sentence **“Raw markdown is the source of truth.”** (not `==highlights==` — that mark decoration is purple, not a CM selection). A CodeMirror capsule (`.cm-tooltip.selection-format-bar` / `.selection-capsule`) **must** appear with comment, lightning, divider, and **X** only. The comment card stays **closed** until you click comment. There are **no** B/I/S/code/link buttons and no Global/Copy on the capsule. Lightning sends the selection immediately (empty comment). Open the card: **Send** stays enabled with an empty textarea and ships `File: …:line-line` plus a fenced copy (no `Comment:` line). Type a note and **Send** — the prompt includes `Comment: …`. Top **Global comment** opens the Global Comment card; **Add** builds a file-level prompt. **Copy** copies the **full document from the host TextDocument** (pill shows Copied only after the host ack; paste must not be empty) and does not open chat. Capsule **X** dismisses the selection UI. Card **X** closes the card only. Escape closes an open card first, then the capsule. Reading mode still shows the capsule if a selection exists. Cmd/Ctrl+B / I / K still wrap with no format icons on the bar. Confirm Cmd/Ctrl+S still saves (not hijacked). Heading/list formats remain available via **Format Selection**.
...
13b. **Reading mode**  
    Toggle reading mode: title icon switches book ↔ pencil. The webview must **not** show a Reading chip or Format strip. The quiet Global comment / Copy pills stay. Selecting text still shows the comment capsule (no format icons). Theme toggle remains in the editor title.
