...
- Feishu outline placement (push rail / icon rail / last-resort overlay), nested heading collapse + prune, YAML frontmatter / thematic-break `---` excluded from headings; no top Format strip; selection-bar show/hide including drawSelection layer fallbacks when `coordsAtPos` and `window.getSelection()` fail; image failure copy
...
11. **Selection format bar / shortcuts**  
    There must be **no** top Format strip, Reading chip, or outline toggle in the webview. Select the plain sentence **“Raw markdown is the source of truth.”** (not `==highlights==` — that mark decoration is purple, not a CM selection). A floating bar **must** appear even if `document.activeElement` is `#root` / `.app` / `.editor-frame` (not BODY), `hasFocus` flickers, `window.getSelection()` is collapsed, or `coordsAtPos` is null. If every coord source fails, the bar still mounts at a safe default — it must not stay hidden while a CM selection highlight is visible. Click bold — markdown wraps and the bar’s pressed state updates. Collapse the selection, click the outline, or switch to reading mode: the bar disappears. Cmd/Ctrl+B / I / K still wrap with no bar on screen. Confirm Cmd/Ctrl+S still saves (not hijacked). Heading/list formats remain available via **Format Selection**.
