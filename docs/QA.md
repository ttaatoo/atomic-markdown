...
- Feishu outline placement (push rail / icon rail / last-resort overlay), nested heading collapse + prune, YAML frontmatter / thematic-break `---` excluded from headings; no top Format strip; CM `showTooltip` comment capsule (card closed by default) plus fixed top Global comment / Copy; chat prompt builder (empty comment + global fence/path-only) and `sendToChat` / `copyText` protocol; image failure copy
