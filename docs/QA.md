...
- Feishu outline placement (push rail / icon rail / last-resort overlay), nested heading collapse + prune, YAML frontmatter / thematic-break `---` excluded from headings; no top Format strip; CM `showTooltip` format bar for a non-empty edit selection; image failure copy
...
11. **Selection format bar / shortcuts**  
    There must be **no** top Format strip, Reading chip, or outline toggle in the webview. Select the plain sentence **“Raw markdown is the source of truth.”** (not `==highlights==` — that mark decoration is purple, not a CM selection). A CodeMirror tooltip (`.cm-tooltip.selection-format-bar`) **must** appear above the selection. It is driven by CM `state.selection.main`, not React/`document.activeElement`. Click bold — markdown wraps and the bar’s pressed state updates. Collapse the selection or switch to reading mode: the bar disappears. Cmd/Ctrl+B / I / K still wrap with no bar on screen. Confirm Cmd/Ctrl+S still saves (not hijacked). Heading/list formats remain available via **Format Selection**.
